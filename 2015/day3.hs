...
updateMapAt :: M.Map Point Int -> Point -> M.Map Point Int
updateMapAt m p =
    M.insert p (1 + M.findWithDefault 0 p m) m

getNextPoint :: Point -> Char -> Point
getNextPoint (Point x y) c
    | c == '^' = Point x $ y + 1
    | c == '>' = Point (x + 1) y
    | c == 'v' = Point x $ y - 1
    | c == '<' = Point (x - 1) y
    | otherwise = error "Bad char at getNextPoint"

housesWithGifts :: String -> M.Map Point Int -> Point -> M.Map Point Int
housesWithGifts [] m p = updateMapAt m p
housesWithGifts (c:cs) m p = housesWithGifts cs (updateMapAt m p) $ getNextPoint p c

data Step = Santa | RoboSanta

housesWithGiftsShared :: String -> M.Map Point Int -> Point -> Point -> Step -> M.Map Point Int
housesWithGiftsShared [] m santa roboSanta Santa = updateMapAt m santa
housesWithGiftsShared [] m santa roboSanta RoboSanta = updateMapAt m roboSanta
housesWithGiftsShared (c:cs) m santa roboSanta Santa = housesWithGiftsShared cs (updateMapAt m santa) santa (getNextPoint roboSanta c) RoboSanta
housesWithGiftsShared (c:cs) m santa roboSanta RoboSanta = housesWithGiftsShared cs (updateMapAt m roboSanta) (getNextPoint santa c) roboSanta Santa
...
main = putStr $ show $ M.size $ housesWithGiftsShared args M.empty emptyPoint emptyPoint Santa
